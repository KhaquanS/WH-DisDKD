...
def save_checkpoint(model, optimizer, epoch: int, accuracy: float, args, 
                   is_best: bool = False):
    ...
    if dataset_name in DOMAINBED_DATASETS or dataset_name in ['PACS', 'PACS_DEEPLAKE']:
        if args.classic_split:
            ...
            domains_str = '_'.join(sorted(args.train_domains)) if args.train_domains else 'all'
            filename = (f'{args.method}_{args.teacher}_to_{args.student}_{args.dataset}_'
                       f'classic_{domains_str}_{suffix}.pth')
        else:
            # OOD setup - separate train/val domains
            train_domains_str = '_'.join(sorted(args.train_domains)) if args.train_domains else 'all'
            val_domains_str = '_'.join(sorted(args.val_domains)) if args.val_domains else 'default'
            filename = (f'{args.method}_{args.teacher}_to_{args.student}_{args.dataset}_'
